...
import top.yztz.gingerbulkmessenger.services.MessageService;
...
public class SendingActivity extends AppCompatActivity implements MessageService.Callback {
    ...
    public void onMessageSubmitted(int index) {
        runOnUiThread(() -> {
            // No longer waiting on a delivery/sent acknowledgement broadcast:
            // as soon as the SMS is handed off, mark it as sent and move on.
    ...
    public void onMessageConfirmed(int index, boolean success) {
        // Intentionally unused: message state is now finalized in onMessageSubmitted
        // without waiting for the radio's sent-confirmation broadcast.
    }
